...
class SimpleRNN:
    ...
    def build(self, We, Wx, Wh, bh, h0, Wo, bo):
        ...
        # print(self.Wx.get_shape())
        # print(self.We.get_shape())
        # print(self.Wh.get_shape())
        # print(self.h0.get_shape())
        # print(XW_Wx.get_shape())

        def recurrence(h_t1, XW_Wx_t):
            # returns h(t), y(t)
            # print(h_t1.get_shape())
        ...
        #h_print = tf.Print(h, [h], 'The value of h is as follows:', summarize=self.V)
        #print(h.get_shape())
        # print(self.Wo.get_shape())

        # output
        logits = tf.matmul(h, self.Wo) + self.bo
        # print('Logits shape is as folllows6')
        # print(logits.get_shape())
        #logits_print = tf.Print(logits, [logits], 'Value of logits is as follows', summarize=self.V)
        prediction = tf.argmax(logits, 1)
        # prediction_print = tf.Print(prediction, [prediction], 'Value of prediction', summarize=self.V)
        self.output_probs = tf.nn.softmax(logits)
        # self.output_probs_print = tf.Print(self.output_probs, [self.output_probs], 'Value of output_probs', summarize=self.V)
        #self.output = self.output_probs_print
    ...
    def fit(self, X, epochs=500, show_fig=False):
        ...
        for i in range(epochs):
            ...
            for j in range(N):
                ...
                #print(input_sequence)
                # self.h1.eval()
                # self.session.run([self.h1], feed_dict={self.tfX: input_sequence, self.tfY: output_sequence})
                # print "p:", p
                cost += c
